...
这是 LeetCode 第 121 题：「买卖股票的最佳时机」（Best Time to Buy and Sell Stock），属于经典的「一次交易」最大利润问题。

### 题目核心
你只能买入一次、卖出一次，且必须「先买后卖」，求最大利润（不能赚钱就返回 0）。
...
### 最优解法：一次遍历 + 记录历史最低价（推荐！）

**核心思想**：
- 遍历每一天时，维护一个「到目前为止见过的最低价格」`minPrice`
- 每来到一个新价格，就计算：如果今天卖出，能赚多少？`prices[i] - minPrice`
- 同时更新最大利润和历史最低价
...
var maxProfit = function(prices) {
    ...
    let minPrice = prices[0];  // 到目前为止的最低价
...
|------|------|----------|-------------------------------|-----------|------|
| 初始 |      | 7        |                               | 0         |      |
| 第2天 | 1    | 1        | 1-7=负，不更新                | 0         | 发现更低价，更新minPrice |
| 第3天 | 5    | 1        | 5-1=4                         | 4         | 刷新最大利润 |
| 第4天 | 3    | 1        | 3-1=2                         | 4         | 不如之前 |
| 第5天 | 6    | 1        | 6-1=5                         | 5         | 最大利润！ |
| 第6天 | 4    | 1        | 4-1=3                         | 5         | 不变 |
...
如果你愿意，我也可以帮你写一个 **加日志版** 的函数（在控制台打印每一步变量的变化），方便你在浏览器里跑一下看效果 👀。
